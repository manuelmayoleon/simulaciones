...
X = np.zeros([int(len(t)/cut), N])
ligand = np.zeros([int(len(t)/cut), N])
...
food_profile = np.zeros(len(x_bin))
jj = 0

for xx in x_bin:
    food_profile[jj] =  step_function(xx,L,c0,c1)
    jj+=1
...
for i in range(len(t)):
    ...
    if i % cut == 0:
        ...
        j=0
        for pos in x[k1]:
            ligand[k1][j] = step_function(pos,L,c0,c1)
            j+=1
        # print(x[k1])
        if k1==0:
            cdot = (ligand[k1])/dt
        else:
            cdot = (ligand[k1] -ligand[k1-1] )/dt
...
plt.plot(x_bin,rho_mean, marker="o",linestyle="")
